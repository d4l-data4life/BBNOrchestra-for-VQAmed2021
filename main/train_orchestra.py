...
if __name__ == "__main__":
    ...
    all_set = eval(cfg.DATASET.DATASET)("train", cfg)
    overallvalid_set = eval(cfg.DATASET.DATASET)("valid", cfg)

    annotations = all_set.get_annotations()
    num_classes = all_set.get_num_classes()
    ...
    for k in range(n_splits):
        logger.info( 
                f"Split {k+1} from total {n_splits} splits"
                )
        train, valid = train_test_split(annotations, test_size = 0.1)
        train_set = eval(cfg.DATASET.DATASETBAGGING)(train, num_classes, "train", cfg)
        valid_set = eval(cfg.DATASET.DATASETBAGGING)(valid, num_classes, "valid", cfg)
        ...
        # close loop
        model_dir = os.path.join(cfg.OUTPUT_DIR, cfg.NAME, f"models_{k}split")
        code_dir = os.path.join(cfg.OUTPUT_DIR, cfg.NAME, f"codes_{k}split")
        tensorboard_dir = (
            os.path.join(cfg.OUTPUT_DIR, cfg.NAME, f"tensorboard_{k}split")
        ...
        best_result, best_epoch, start_epoch, best_model = 0, 0, 1, ""
        # ----- BEGIN RESUME ---------
        ...
        if cfg.TRAIN.TENSORBOARD.ENABLE:
            writer.close()
        logger.info(
            "-------------------Train Finished_ Split {} :{}-------------------".format(k, cfg.NAME)
        )
        members.append(best_model)
        scores.append(best_result)
    for k in range(n_splits):
        logger.info(
            "-------------------Best score in split {} :{}-------------------".format(k, scores[k]*100)
        )

    avg_split_acc = sum(scores)*100 / len(scores)
    logger.info(
            "-------------------Average val accuracy over splits : {}-------------------".format(avg_split_acc)
        )
